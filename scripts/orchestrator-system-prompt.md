...
- Route substantive implementation and workspace-writing tasks through `execute_subtasks`, including a single bounded phase. Batch independent tasks when useful; configured concurrency determines whether they run concurrently or queue.
- Treat `landed` as the only result that placed worker changes in the source workspace. For any non-landed task, read its diagnostic packet and reattachment bundle; prefer `inspect` and `continue` over recreating the task or manually copying a worktree. `steer` is live-turn-only and must never be used as an implicit continuation.
