...
export const PI_SETTLEMENT_SECRET_ENV = "PI_REVIEW_GATE_SETTLEMENT_SECRET";
export const PI_SETTLEMENT_PATH_ENV = "PI_REVIEW_GATE_SETTLEMENT_PATH";
export const PI_SETTLEMENT_SESSION_ENV = "PI_REVIEW_GATE_SETTLEMENT_SESSION";
export const PI_SETTLEMENT_CHILD_ENV = "PI_REVIEW_GATE_SETTLEMENT_CHILD";

// Incompatible with the old zero-browser-ownership attestation. Never accept
// v1 receipts or legacy environment bootstrap as evidence of this contract.
const RECEIPT_VERSION = 2;
const MAX_RECEIPT_BYTES = 2_048;
const TOKEN = /^[A-Za-z0-9_-]{16,200}$/;

export interface PiSettlementBootstrap {
  path: string;
  sessionId: string;
  childId: string;
  secret: string;
...
export function createPiSettlementBootstrap(artifactDir: string, sessionId: string): PiSettlementBootstrap {
...
export function piSettlementEnvironment(bootstrap: PiSettlementBootstrap): NodeJS.ProcessEnv {
  return {
    [PI_SETTLEMENT_SECRET_ENV]: bootstrap.secret,
    [PI_SETTLEMENT_PATH_ENV]: bootstrap.path,
    [PI_SETTLEMENT_SESSION_ENV]: bootstrap.sessionId,
    [PI_SETTLEMENT_CHILD_ENV]: bootstrap.childId,
  };
}
 ...
 */
export function capturePiSettlementBootstrap(env: NodeJS.ProcessEnv = process.env): PiSettlementBootstrap | undefined {
  const secret = env[PI_SETTLEMENT_SECRET_ENV];
  const path = env[PI_SETTLEMENT_PATH_ENV];
  const sessionId = env[PI_SETTLEMENT_SESSION_ENV];
  const childId = env[PI_SETTLEMENT_CHILD_ENV];
  delete env[PI_SETTLEMENT_SECRET_ENV];
  delete env[PI_SETTLEMENT_PATH_ENV];
  delete env[PI_SETTLEMENT_SESSION_ENV];
  delete env[PI_SETTLEMENT_CHILD_ENV];
  // Old parent/new child is unsupported, and must not leak a legacy secret
  // into model-spawned tools while failing to acknowledge that parent.
  for (const suffix of ["SECRET", "PATH", "SESSION", "CHILD"]) delete env[`PI_REVIEW_GATE_QUIESCENCE_${suffix}`];
  ...
  if (
    ...
    throw new Error("Pi executor settlement bootstrap is malformed.");
  }
  const expectedSuffix = join("executor-acks", `${childId}.json`);
  if (!resolve(path).endsWith(expectedSuffix)) throw new Error("Pi executor settlement receipt path is invalid.");
  return { path: resolve(path), sessionId, childId, secret, pid: process.pid };
}

/** Authenticate agent_settled only. The browser is explicitly still live. */
export async function publishPiSettlementReceipt(
  ...
  const receipt: PiSettlementReceipt = { ...unsigned, mac: receiptMac(bootstrap.secret, unsigned) };
  const body = `${JSON.stringify(receipt)}\n`;
  if (Buffer.byteLength(body) > MAX_RECEIPT_BYTES) throw new Error("Pi executor settlement receipt exceeds its bound.");
 ...
 */
export async function awaitPiSettlementReceipt(
  bootstrap: PiSettlementBootstrap,
  ...
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1) throw new Error("Pi settlement acknowledgement timeout is invalid.");
  ...
  for (;;) {
    ...
    if (remaining <= 0) throw new Error("Pi executor settlement acknowledgement was not received before its bounded deadline.");
    await delay(Math.min(20, remaining), signal);
  }
}

export async function removePiSettlementReceipt(bootstrap: PiSettlementBootstrap): Promise<void> {
  await unlink(bootstrap.path).catch((error: NodeJS.ErrnoException) => {
    if (error.code !== "ENOENT") throw error;
  });
}

function parseReceipt(raw: string): PiSettlementReceipt {
  let value: unknown;
  try { value = JSON.parse(raw); } catch { throw new Error("Pi executor settlement acknowledgement is malformed."); }
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Pi executor settlement acknowledgement is malformed.");
...
function verifyReceipt(receipt: PiSettlementReceipt, bootstrap: PiSettlementBootstrap, afterSettlement: number): void {
  ...
  if (!/^[A-Za-z0-9_-]{43}$/.test(receipt.mac)) {
    throw new Error("Pi executor settlement acknowledgement signature is malformed.");
  }
...
function receiptMac(
  masterSecret: string,
  receipt: Omit<PiSettlementReceipt, "mac">,
): string {
  ...
  const oneShot = createHmac("sha256", masterSecret)
    .update(`pi-review-gate-live-browser-settlement-key:v2:${receipt.settlement}`)
    .digest();
