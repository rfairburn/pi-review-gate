...
test("PiJsonlActivityExtractor emits bounded useful lifecycle summaries instead of token deltas", () => {
  ...
  const stream = [
    ...
    JSON.stringify({ type: "message_update", assistantMessageEvent: { type: "text_end", content: "I’ll inspect the tests,\nthen update the implementation." } }),
    JSON.stringify({ type: "message_end", message: { role: "assistant", content: [{ type: "text", text: "I’ll inspect the tests,\nthen update the implementation." }] } }),
...
test("CodexJsonlActivityExtractor emits useful live milestones from split JSONL chunks", () => {
  ...
  const stream = [
    ...
    JSON.stringify({ type: "item.completed", item: { type: "agent_message", text: "I’ll inspect the tests,\nthen update the implementation." } }),
