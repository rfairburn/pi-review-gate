...
import {
  assertPinnedServerAddress,
  browserRouteDecision,
  chromiumDnsPinningArgs,
  chromiumHostResolverRules,
  crossOriginBlockedMessage,
  finalizeBrowserNetwork,
...
// ---------------------------------------------------------------------------
// Browser pinning: host-resolver rules and route policy.
// ---------------------------------------------------------------------------

test("chromium launch arguments enforce the DNS pin and disable proxying", () => {
  const validated = { href: "http://example.test/", hostname: "example.test", addresses: [publicResolverAnswer] };
  assert.deepEqual(chromiumDnsPinningArgs(validated), [
    "--no-proxy-server",
    `--host-resolver-rules=MAP example.test ${publicResolverAnswer}, MAP * ~NOTFOUND`,
  ]);
});

test("chromium host-resolver rules pin the validated hostname and default-deny every other endpoint", () => {
  assert.equal(
    chromiumHostResolverRules({ href: "http://example.test/", hostname: "example.test", addresses: [publicResolverAnswer, "2606:4700:4700::1111"] }),
    `MAP example.test ${publicResolverAnswer}, MAP * ~NOTFOUND`,
  );
  // IPv6-only answers pin a bracketed literal; IPv4 is preferred when both exist.
  assert.equal(
    chromiumHostResolverRules({ href: "http://example.test/", hostname: "example.test", addresses: ["2606:4700:4700::1111"] }),
    "MAP example.test [2606:4700:4700::1111], MAP * ~NOTFOUND",
  );
  assert.equal(preferredPinnedAddress(["2606:4700:4700::1111", publicResolverAnswer]), publicResolverAnswer);
  assert.equal(preferredPinnedAddress(["2606:4700:4700::1111"]), "2606:4700:4700::1111");
  assert.throws(
    () => chromiumHostResolverRules({ href: "http://example.test/", hostname: "example.test", addresses: [] }),
    /No validated public address/,
  );
  // IP-literal targets pin themselves; no DNS is involved.
  assert.equal(
    chromiumHostResolverRules({ href: "http://2606:4700:4700::1111/", hostname: "2606:4700:4700::1111", addresses: ["2606:4700:4700::1111"] }),
    "MAP 2606:4700:4700::1111 [2606:4700:4700::1111], MAP * ~NOTFOUND",
  );
});

test("browser route policy allows only pinned-hostname HTTP(S) and narrowly-allowed local protocols", () => {
  const host = "example.com";
  // Same-hostname HTTP(S) is allowed for every scheme and port; the Chromium
  // MAP rule pins the hostname and each response's peer address is verified
  // separately.
  assert.deepEqual(browserRouteDecision("document", "https://example.com/page?next=1", host), { allowed: true });
  assert.deepEqual(browserRouteDecision("xhr", "https://example.com:443/api", host), { allowed: true });
  assert.deepEqual(browserRouteDecision("fetch", "http://example.com/insecure", host), { allowed: true });
  // The very common http→https upgrade redirect on the same host stays allowed
  // (the page was http://example.com and redirected to https).
  assert.deepEqual(browserRouteDecision("document", "https://example.com/secure", "example.com"), { allowed: true });
  // IPv6-literal hosts compare without URL brackets, matching ValidatedUrl.hostname.
  assert.deepEqual(browserRouteDecision("document", "http://[2606:4700:4700::1111]/page", "2606:4700:4700::1111"), { allowed: true });
  assert.equal(browserRouteDecision("document", "http://[2606:4700:4700::2222]/page", "2606:4700:4700::1111").allowed, false);
  assert.equal(
    browserRouteDecision("document", "http://[2606:4700:4700::2222]/page", "2606:4700:4700::1111").crossOriginBlock,
    "http://[2606:4700:4700::2222]/page",
  );
  // Local browser protocols are narrowly allowed.
  assert.deepEqual(browserRouteDecision("document", "about:blank", host), { allowed: true });
  assert.deepEqual(browserRouteDecision("document", "data:text/html,hi", host), { allowed: true });
  assert.deepEqual(browserRouteDecision("document", "blob:https://example.com/uuid", host), { allowed: true });
  // A different hostname cannot be pinned and is blocked and reported.
  const cross = browserRouteDecision("document", "https://other.test/path", host);
  assert.equal(cross.allowed, false);
  assert.equal(cross.crossOriginBlock, "https://other.test/path");
  // Non-HTTP protocols are blocked as unpinnable.
  const javascript = browserRouteDecision("document", "javascript:alert(1)", host);
  assert.equal(javascript.allowed, false);
  assert.equal(javascript.crossOriginBlock, "javascript:alert(1)");
  // Cross-origin passive resources are reported, not silently dropped.
  const crossImage = browserRouteDecision("image", "https://cdn.other.test/a.png", host);
  assert.equal(crossImage.allowed, false);
  assert.equal(crossImage.crossOriginBlock, "https://cdn.other.test/a.png");
  // Same-hostname passive resources stay blocked by policy but do not taint.
  assert.deepEqual(browserRouteDecision("image", "https://example.com/a.png", host), { allowed: false });
  assert.deepEqual(browserRouteDecision("font", "https://example.com:8443/f.woff2", host), { allowed: false });
  // Unparseable URLs fail closed.
  assert.equal(browserRouteDecision("xhr", "not a url", host).allowed, false);
});

test("pinned server-address verification accepts only the pinned validated peer", () => {
  const pinned = "203.0.114.1";
  // Exact match passes.
  assert.doesNotThrow(() => assertPinnedServerAddress("https://example.com/page", { ipAddress: pinned, port: 443 }, pinned));
  // Brackets and equivalent compressed/expanded IPv6 spellings are normalized
  // before comparison.
  assert.doesNotThrow(() => assertPinnedServerAddress("https://example.com/page", { ipAddress: "[2606:4700:4700::1111]", port: 443 }, "2606:4700:4700::1111"));
  assert.doesNotThrow(() => assertPinnedServerAddress(
    "https://example.com/page",
    { ipAddress: "2606:4700:4700:0:0:0:0:1111", port: 443 },
    "2606:4700:4700::1111",
  ));
  // Non-HTTP(S) URLs are skipped without verification.
  assert.doesNotThrow(() => assertPinnedServerAddress("about:blank", null, pinned));
  assert.doesNotThrow(() => assertPinnedServerAddress("data:text/html,hi", null, pinned));
  // Blocked peer addresses throw even if they matched a different pin.
  assert.throws(
    () => assertPinnedServerAddress("https://example.com/page", { ipAddress: "127.0.0.1", port: 443 }, "127.0.0.1"),
    /came from 127\.0\.0\.1/,
  );
  assert.throws(
    () => assertPinnedServerAddress("https://example.com/page", { ipAddress: "169.254.169.254", port: 443 }, "169.254.169.254"),
    /came from 169\.254\.169\.254/,
  );
  // A public-but-different peer fails closed.
  assert.throws(
    () => assertPinnedServerAddress("https://example.com/page", { ipAddress: "198.20.0.5", port: 443 }, pinned),
    /came from 198\.20\.0\.5, not the pinned validated address/,
  );
  // A missing server address refuses the unverifiable render.
  assert.throws(
    () => assertPinnedServerAddress("https://example.com/page", null, pinned),
    /exposed no server address/,
  );
  // Malformed response URLs fail closed.
  assert.throws(() => assertPinnedServerAddress("not a url", null, pinned), /invalid/);
});

test("cross-origin browser blocks carry actionable compatibility text", () => {
  const message = crossOriginBlockedMessage("https://other.test/final");
  assert.match(message, /could not be pinned/);
  assert.match(message, /fails closed/);
  assert.match(message, /extract that final URL directly/);
  assert.match(message, /https:\/\/other\.test\/final/);
  const bounded = crossOriginBlockedMessage(`https://other.test/${"x".repeat(10_000)}`);
  assert.ok(bounded.length < 2_000, "attacker-controlled browser URLs must be bounded in diagnostics");
  assert.match(bounded, /… \(truncated\)/);
});

test("browser finalization awaits address checks started during shutdown", async () => {
  let violation: Error | undefined;
  const pending: Promise<void>[] = [];
  await assert.rejects(
    finalizeBrowserNetwork(
      async () => {
        // Simulates a response arriving while browser networking is closing:
        // its check is appended during closure and must still be honored.
        pending.push(Promise.resolve().then(() => {
          violation = new Error("late blocked peer");
        }));
      },
      pending,
      () => violation,
  ...
  // A clean shutdown with no violation resolves.
  await assert.doesNotReject(finalizeBrowserNetwork(async () => undefined, [], () => undefined));
});

test("BrowserExtract reports the compatibility text through renderWithChromium policy errors", async () => {
  // No Chromium is needed: validation runs first and the injected resolver
  // answers private, so renderWithChromium fails closed before launching.
  await assert.rejects(
    renderWithChromium("http://private.test/", testOptions({ resolveHostname: () => Promise.resolve(["127.0.0.1"]) })),
    /non-public address/,
  );
  // An unresolvable hostname also fails before any browser launch.
  await assert.rejects(
...
// the resolver rule string is driven through a real browser against loopback
// listeners. The pinned address is a test-only stand-in for a validated public
// address routed locally; production pins always come from validatePublicUrl.
// ---------------------------------------------------------------------------

const chromiumInstalled = existsSync(chromium.executablePath());

function denyAllExceptPinnedServer(): Server {
  const server = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/plain" });
    response.end("pinned-ok");
  });
  server.on("connection", () => {
    (server as Server & { connectionCount?: number }).connectionCount =
      ((server as Server & { connectionCount?: number }).connectionCount ?? 0) + 1;
  });
  return server;
}

function connectionsOf(server: Server): number {
  return (server as Server & { connectionCount?: number }).connectionCount ?? 0;
}

test(
  "Chromium renders the pinned hostname and receives zero connections from an unpinned direct-IP endpoint",
  { skip: !chromiumInstalled },
  async () => {
    const pinnedServer = denyAllExceptPinnedServer();
    const blockedServer = denyAllExceptPinnedServer();
    const port = await listen(pinnedServer);
    const blockedPort = await listen(blockedServer);
    const browser = await chromium.launch({
      headless: true,
      timeout: 15_000,
      args: chromiumDnsPinningArgs({ href: `http://allowed.test:${port}/`, hostname: "allowed.test", addresses: ["127.0.0.1"] }),
    });
    try {
      const page = await (await browser.newContext()).newPage();
      // Positive control: the admitted hostname resolves through the MAP rule
      // and actually connects to the pinned address.
      const response = await page.goto(`http://allowed.test:${port}/`, { timeout: 10_000 });
      assert.equal(response?.status(), 200);
      assert.equal(await page.content().then((html) => html.includes("pinned-ok")), true);
      const pinnedConnections = connectionsOf(pinnedServer);
      assert.ok(pinnedConnections >= 1, "the pinned endpoint must be reachable");
      // No route interception is installed here. A direct-IP destination —
      // the route-blind preconnect and alternative-service vector — must be
      // denied by the browser's resolver-level default denial itself
      // (`MAP * ~NOTFOUND` matches IP literals above the resolver manager),
      // not by a reserved nonresolving name or reactive checks.
      await assert.rejects(page.goto(`http://127.0.0.1:${blockedPort}/`, { timeout: 10_000 }), /ERR_NAME_NOT_RESOLVED/);
      assert.equal(connectionsOf(blockedServer), 0, "an unpinned direct-IP endpoint must receive zero connections");
    } finally {
      await browser.close();
      await Promise.all([close(pinnedServer), close(blockedServer)]);
    }
  },
);